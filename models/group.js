...
module.exports = class Group {
	...
	save() {
		return db.execute(
			'INSERT INTO `societyboard`.groups (societyId, user1Email, user2Email, user3Email) VALUES (?, ?, ?, ?)',
	...
	static findSocietyIdByGroupId(groupId) {
		return db.execute(
			'SELECT societyId FROM `societyboard`.groups WHERE groups.groupId = ?',
			[groupId]
		);
	}
	...
	static findGroupIdBySocietyId(societyId) {
		return db.execute(
			'SELECT groupId FROM `societyboard`.groups WHERE groups.societyId = ?',
			[societyId]
		);
	}
	...
	static findGroupIdByUser1Email(user1Email) {
		return db.execute(
			'SELECT groupId FROM `societyboard`.groups WHERE groups.user1Email = ?',
			[user1Email]
		);
	}
};
